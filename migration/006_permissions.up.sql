...
-- ============================
CREATE TABLE IF NOT EXISTS edev_core_tenants (
...
CREATE TABLE IF NOT EXISTS edev_core_tenant_members (
  id INTEGER PRIMARY KEY,
  tenant_id INTEGER NOT NULL REFERENCES edev_core_tenants(id) ON DELETE CASCADE,
...
-- ============================
CREATE TABLE IF NOT EXISTS edev_core_groups (
  id INTEGER PRIMARY KEY,
  tenant_id INTEGER NOT NULL REFERENCES edev_core_tenants(id) ON DELETE CASCADE,
  name TEXT NOT NULL,
...
CREATE INDEX IF NOT EXISTS ix_groups_by_tenant ON edev_core_groups(tenant_id);

-- ============================
-- Group memberships (scoped by tenant)
-- ============================
CREATE TABLE IF NOT EXISTS edev_core_group_members (
  id INTEGER PRIMARY KEY,
  tenant_id INTEGER NOT NULL REFERENCES edev_core_tenants(id) ON DELETE CASCADE,
  group_id  INTEGER NOT NULL REFERENCES edev_core_groups(id)  ON DELETE CASCADE,
  user_id   INTEGER NOT NULL REFERENCES edev_core_users(id)   ON DELETE CASCADE,
  created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
  UNIQUE(tenant_id, group_id, user_id)
);

CREATE INDEX IF NOT EXISTS ix_group_members_by_user  ON edev_core_group_members(tenant_id, user_id);
CREATE INDEX IF NOT EXISTS ix_group_members_by_group ON edev_core_group_members(tenant_id, group_id);
...
-- ============================
CREATE TABLE IF NOT EXISTS edev_core_permits (
  id INTEGER PRIMARY KEY,
  tenant_id INTEGER NOT NULL REFERENCES edev_core_tenants(id) ON DELETE CASCADE,
  user_id  INTEGER REFERENCES edev_core_users(id)  ON DELETE CASCADE,
  group_id INTEGER REFERENCES edev_core_groups(id) ON DELETE CASCADE,
...
CREATE UNIQUE INDEX IF NOT EXISTS ux_permits_user
  ON edev_core_permits(tenant_id, user_id,  resource, scope)
  WHERE user_id IS NOT NULL;

CREATE UNIQUE INDEX IF NOT EXISTS ux_permits_group
  ON edev_core_permits(tenant_id, group_id, resource, scope)
  WHERE group_id IS NOT NULL;

-- Lookups (quick queries by tenant/resource/scope)
CREATE INDEX IF NOT EXISTS ix_permits_lookup_user
  ON edev_core_permits(tenant_id, resource, scope, user_id);

CREATE INDEX IF NOT EXISTS ix_permits_lookup_group
  ON edev_core_permits(tenant_id, resource, scope, group_id);

CREATE INDEX IF NOT EXISTS ix_permits_scope_null
  ON edev_core_permits(tenant_id, resource)
  WHERE scope IS NULL;

-- Admin-oriented extra lookups
CREATE INDEX IF NOT EXISTS ix_permits_by_user  ON edev_core_permits(user_id);
CREATE INDEX IF NOT EXISTS ix_permits_by_group ON edev_core_permits(group_id);

-- ============================
-- Permissions dictionary (for admin UI / documentation)
-- Optional hierarchy via parent_id (purely organizational)
-- ============================
CREATE TABLE IF NOT EXISTS edev_core_resources (
  id INTEGER PRIMARY KEY,
  parent_id INTEGER REFERENCES edev_core_resources(id) ON DELETE SET NULL, -- hierarchy for UI grouping
...
CREATE INDEX IF NOT EXISTS ix_resources_parent ON edev_core_resources(parent_id);

-- ============================
-- Effective permits view (per-tenant):
-- Combines direct user permits and group-based permits into a single stream.
-- ============================
CREATE VIEW IF NOT EXISTS edev_core_effective_permits AS
    -- Direct user permits
    SELECT
    ...
    SELECT
        ...
        p.created_at AS created_at
    FROM edev_core_permits p
    JOIN edev_core_group_members gm
...
-- SELECT 1
--  FROM edev_core_effective_permits
...
-- INSERT INTO edev_core_permits (tenant_id, group_id, resource, scope, allowed)
--   VALUES (100, 1, 'menu.admin', NULL, 1);

-- Game master (user 5) can invite players to table t1 in tenant 100:
-- INSERT INTO edev_core_permits (tenant_id, user_id, resource, scope, allowed)
...
-- ============================
-- Notes
-- - Keep “implicit owner” out of edev_core_permits (domain rule). For objects with owner_user_id,
